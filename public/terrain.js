...
export function terrainSummary(icon,terrain){
 ...
 return `<section class="tile-terrain" aria-label="${terrainNames[terrain]} terrain effects"><div class="tile-terrain-heading"><button class="tile-terrain-icon" data-terrain-help="${terrainIcons[terrain]}" aria-label="${terrainNames[terrain]}">${icon(terrainIcons[terrain])}</button></div><div class="tile-effects">${effects.map(([tone,symbol,value,label])=>`<button class="tile-effect ${tone}" data-terrain-help="${terrainIcons[terrain]}" aria-label="${label}">${icon(symbol)}<span>${value}</span></button>`).join('')}</div></section>`;
}
