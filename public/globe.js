import {attackTiming,drawAttack,unitScales} from './combat-visuals.js';
import {RiggedMesh,Woodland} from './model-assets.js';
import {coastalEdges,shoreDistance,beachWeight} from './terrain.js';
...
const ridge=p=>{const q=[p[0]*18+p[2]*7,p[1]*25,p[2]*17-p[0]*5],r=1-Math.abs(noise3(q)*2-1);return Math.pow(r,3)*.76+Math.pow(1-Math.abs(noise3(mul(q,2.1))*2-1),4)*.18+noise3(mul(p,120))*.06;};
...
if(vMaterial.y<.5&&mode<.5){
 float patches=fbm(vWorld*95.),detail=fbm(vWorld*460.);float slope=1.-max(0.,dot(n,normalize(vPos)));float rock=max(smoothstep(.024,.065,length(vWorld)-1.),smoothstep(.08,.42,slope));
 ...
 float snow=smoothstep(1.052,1.072,length(vWorld)+patches*.005)*smoothstep(.25,.75,dot(normalize(vNormal),normalize(vPos)));
 base=mix(base,vec3(.80,.84,.82),snow);
 float striation=.9+.1*sin(vWorld.y*650.+patches*8.);base*=mix(.82+detail*.32,striation,rock);
 float beach=clamp(vMaterial.x,0.,1.);vec3 sand=mix(vec3(.32,.25,.13),vec3(.63,.49,.28),smoothstep(1.001,1.010,length(vWorld)));base=mix(base,sand*(.93+detail*.12),beach);
 vec3 face=normalize(cross(dFdx(vPos),dFdy(vPos)));if(dot(face,n)<0.)face=-face;vec3 rockNormal=normalize(mix(n,face,rock*.55));
...
export class Globe {
 constructor(canvas,onSelect){
  ...
  this.u=Object.fromEntries(['camera','viewport','offset','mode','time','oceanNormal','oceanRough','fogTexture','preview','weather[0]','modelBasis','modelOrigin','modelScale','modelTint','modelFlash','skinned','bones[0]','foliage','treeTexture'].map(n=>[n,g.getUniformLocation(this.program,n)]));
  ...
  this.loadModel=kind=>{
   ...
   const name=['guard','cavalry','archer','tank','artillery','recon','drone','fleet','submarine','carrier','engineer','launcher','scout','settler','missile','nuke','satellite'][kind];if(!name)return;
   this.modelLoads.set(kind,performance.now());
   fetch(`/assets/${name}.${[0,2,12].includes(kind)?'rig':'mesh'}`).then(r=>{if(!r.ok)throw Error('Model unavailable');return r.arrayBuffer();}).then(buffer=>{
    if([0,2,12].includes(kind)){this.models[kind]=new RiggedMesh(g,buffer);return;}
    const view=new DataView(buffer),data=[];data.materials=[];
    for(let i=0;i<buffer.byteLength;i+=16){for(let j=0;j<6;j++)data.push(view.getInt16(i+j*2,true)/32767);for(let j=12;j<15;j++)data.push(view.getUint8(i+j)/255);data.materials.push(1,2,0);}
    const mesh=new Mesh(g);mesh.update(data);this.models[kind]=mesh;
   }).catch(e=>console.warn(e));
  };

  this.treeInstances=[];
  Promise.all([fetch('/assets/woodland.mesh').then(r=>{if(!r.ok)throw Error('Trees unavailable');return r.arrayBuffer();}),new Promise((resolve,reject)=>{const img=new Image();img.onload=()=>resolve(img);img.onerror=reject;img.src='/assets/woodland.webp';})]).then(([buffer,img])=>{const texture=g.createTexture();g.activeTexture(g.TEXTURE3);g.bindTexture(g.TEXTURE_2D,texture);g.texImage2D(g.TEXTURE_2D,0,g.RGBA,g.RGBA,g.UNSIGNED_BYTE,img);g.generateMipmap(g.TEXTURE_2D);g.texParameteri(g.TEXTURE_2D,g.TEXTURE_MIN_FILTER,g.LINEAR_MIPMAP_LINEAR);g.texParameteri(g.TEXTURE_2D,g.TEXTURE_WRAP_S,g.CLAMP_TO_EDGE);g.texParameteri(g.TEXTURE_2D,g.TEXTURE_WRAP_T,g.CLAMP_TO_EDGE);this.woodland=new Woodland(g,buffer,texture);this.woodland.instances=this.treeInstances;}).catch(e=>console.warn(e));
 ...
 rebuild(){if(!this.world.length)return;const land=[],lines=[],props=[],air=[],clouds=[],staticBuild=!this.staticReady;
  if(staticBuild)this.treeInstances=[];
  ...
  this.world.forEach((t,i)=>{
   ...
   const drawGround=(a,b,c,fa,fb,fc,ma,mb,mc,da,db,dc,level)=>{
    ...
    tri(land,...ps,col);for(let k=0;k<3;k++){if(t.terrain>0){const p=[a,b,c][k],mountain=[ma,mb,mc][k],east=norm(cross(Math.abs(p[1])>.98?[1,0,0]:[0,1,0],p)),north=cross(p,east),ep=norm(add(p,mul(east,.0007))),np=norm(add(p,mul(north,.0007))),base=surface(p,mountain),dx=(surface(ep,mountain)-base)/.0007,dz=(surface(np,mountain)-base)/.0007,normal=norm(add(add(p,mul(east,-dx)),mul(north,-dz)));for(let j=0;j<3;j++)land[(at+k)*9+3+j]=normal[j];}land.materials[(at+k)*3]=t.terrain===0?0:beachWeight(shoreDistance([a,b,c][k],shores));land.materials[(at+k)*3+2]=t.terrain===0?Math.min(1,shoreDistance([a,b,c][k],shores)/.055):t.terrain;}
   };
   for(let k=0;k<poly.length;k++){
    ...
    if(staticBuild){drawGround(t.p,q,r,fogValue(i),f[0]/f[1],h[0]/h[1],t.terrain===4?1:0,a[0]/a[2],b[0]/b[2],t.terrain===0?1:0,a[1]/a[2],b[1]/b[2],t.terrain===4||shores.length?3:2);
  ...
  });if(staticBuild){if(this.woodland)this.woodland.instances=this.treeInstances;this.land.update(land);this.air.update(air);this.clouds.update(clouds);this.staticReady=true;}this.lines.update(lines);this.props.update(props);this.rebuildTerritory();this.rebuildSelection();
 }
 rebuildTerritory(){
 ...
 updateEffects(now){
  ...
  for(const e of this.effects.items){
   ...
   if(e.action==='shot'){
    drawAttack(e,now,{a,b,base,ring,ribbon,triangle,norm,add,mul,mix,cross});
 ...
 drawModels(now){const g=this.gl,elapsed=Math.min(2,(now-(this.receivedAt||now))/1000);
  const draw=(kind,up,forward,radius,scale,owner,flash=0,animation)=>{const mesh=this.models[kind];if(!mesh)return;let east=norm(cross(Math.abs(up[1])>.98?[1,0,0]:[0,1,0],up)),north=cross(up,east);if(forward&&Math.hypot(...forward)>.001){north=mul(norm(add(forward,mul(up,-forward.reduce((a,v,i)=>a+v*up[i],0)))),-1);east=norm(cross(up,north));}g.uniformMatrix3fv(this.u.modelBasis,false,[...east,...up,...north]);g.uniform3fv(this.u.modelOrigin,add(mul(up,radius),forward?mul(norm(forward),animation?.kick||0):[0,0,0]));g.uniform1f(this.u.modelScale,scale);g.uniform1f(this.u.modelFlash,flash);g.uniform3fv(this.u.modelTint,palette[owner]||[.65,.27,.18]);mesh.draw(g.TRIANGLES,this.u,animation);};
 ...
 frame(now){this.clock=now/1000;const c=this.canvas,g=this.gl;const dpr=Math.min(devicePixelRatio||1,innerWidth<=760?1.4:1.8),w=Math.floor(c.clientWidth*dpr),h=Math.floor(c.clientHeight*dpr);if(c.width!==w||c.height!==h){c.width=w;c.height=h;g.viewport(0,0,w,h);this.targetOffset=this.preview&&innerWidth>760?.24:0;}
  ...
  g.enable(g.DEPTH_TEST);g.disable(g.CULL_FACE);g.enable(g.BLEND);g.blendFunc(g.SRC_ALPHA,g.ONE_MINUS_SRC_ALPHA);g.depthMask(false);g.uniform1f(this.u.mode,2);this.air.draw(g.TRIANGLES);g.depthMask(true);g.disable(g.BLEND);g.uniform1f(this.u.mode,0);this.land.draw(g.TRIANGLES);g.enable(g.BLEND);g.depthMask(false);g.uniform1f(this.u.mode,4);this.territories.draw(g.TRIANGLES);g.disable(g.DEPTH_TEST);this.territoryBorders.draw(g.TRIANGLES);g.enable(g.DEPTH_TEST);g.depthMask(true);g.disable(g.BLEND);g.uniform1f(this.u.mode,0);this.woodland?.draw(this);this.props.draw(g.TRIANGLES);g.uniform1f(this.u.mode,1);this.lines.draw(g.LINES);this.selection.draw(g.TRIANGLES);this.updateRoutes(now);this.routes.draw(g.LINES);
