...
export class Globe {
 ...
 updateUnits(now){const data=[],elapsed=Math.min(2,(now-(this.receivedAt||now))/1000);const tri=(a,b,c,col)=>{const n=norm(cross(add(b,mul(a,-1)),add(c,mul(a,-1))));for(const p of[a,b,c])data.push(...p,...n,...col);};
  ...
  for(const s of this.state?.squads||[]){if(s.boarded_on!=null)continue;const{p}=this.unitPosition(s,now);const col=palette[s.owner]||[.75,.35,.22];ring(p,s.kind===9?.045:.024,col,this.world[s.tile].terrain===0?1.008:ground(p,this.world[s.tile].terrain===4?1:0)+.003);
