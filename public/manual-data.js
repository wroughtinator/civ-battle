...
export const manualEntries={
 ...
 board:['Passengers aboard','The count shows occupied berths and capacity. Move a ground unit onto a friendly carrier to board. Up to three units can travel aboard it. Passengers cannot act independently and die with their transport, including if it is disbanded. Unload passengers before refitting to a ship with fewer berths.'],
 disembark:['Disembark one passenger','Select the carrier, choose Disembark, then tap highlighted adjacent empty land. The first passenger that can enter that terrain lands there, keeping its health and receiving its normal movement cooldown. Tanks and cavalry cannot land on mountains. Unloading stops the carrier and commits it for two seconds.'],
 route:['Move','Select a unit, choose this first action, then tap a destination on the globe. A route appears and the unit follows it one hex at a time. There is no confirmation. Only one unit fits on a hex; moving never attacks. Land units can enter water only by moving onto a friendly boardable ship with a free berth. Carriers have three berths. Passengers travel with the ship, cannot act, and die if it is destroyed. Each step immediately occupies the next hex, with a short visual glide followed by a cooldown. Terrain, weather and unit speed change the cooldown. A queued move waits for an ongoing action to finish. Tap the selected action again to leave targeting.'],
...
const roles=[
 ...
 'Large naval platform with attack range up to three hexes. Carries up to three ground units; use Move to board and Disembark to unload one onto adjacent empty land. Strong against drones. Can activate an air-defence screen or ping sonar. Submarines and artillery threaten it.',
