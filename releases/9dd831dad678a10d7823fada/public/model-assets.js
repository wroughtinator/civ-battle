...
export async function loadTexture(gl,name,unit=3){
 const img=new Image();await new Promise((resolve,reject)=>{img.onload=resolve;img.onerror=()=>reject(Error(`Texture unavailable: ${name}`));img.src=`/releases/9dd831dad678a10d7823fada/assets/forge/${name}.png`;});
