...
export class Soundscape {
 ...
 async unlock(){if(!this.enabled)return;try{
  ...
  this.loading=true;const [manifest,response]=await Promise.all([fetch('/releases/e7e94a0aecc14a1c2844ca15/assets/audio/cues.json').then(r=>r.json()),fetch('/releases/e7e94a0aecc14a1c2844ca15/assets/audio/game-audio.mp3')]);this.cues=manifest;this.buffer=await this.ctx.decodeAudioData(await response.arrayBuffer());this.ready=true;
