...
export class Globe {
 constructor(canvas,onSelect){
  ...
  this.loadModel=kind=>{
   ...
   trackLoad(Promise.all([assetBytes('/releases/d6c0b53103cc185548d09c34/assets/forge/'+name+'.rig'),loadTexture(g,name)]).then(([buffer,texture])=>this.models[kind]=new RiggedMesh(g,buffer,texture))).catch(e=>console.warn(e));
  };
  ...
  this.loadStatic=(name,sway=false)=>{
   ...
   trackLoad(Promise.all([assetBytes('/releases/d6c0b53103cc185548d09c34/assets/forge/'+name+'.mesh'),loadTexture(g,name)]).then(([buffer,texture])=>this.staticMeshes.set(name,new Woodland(g,buffer,texture,sway,name.startsWith('discovery-'))))).catch(e=>console.warn(e));
  };
