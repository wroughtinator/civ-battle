...
export class Globe {
 constructor(canvas,onSelect){
  ...
  this.loadModel=kind=>{
   ...
   Promise.all([fetch('/releases/b0e0db9d9b0b547e08817056/assets/forge/'+name+'.rig').then(r=>{if(!r.ok)throw Error('Model unavailable: '+name);return r.arrayBuffer();}),loadTexture(g,name)]).then(([buffer,texture])=>this.models[kind]=new RiggedMesh(g,buffer,texture)).catch(e=>console.warn(e));
  };
  ...
  this.loadStatic=(name,sway=false)=>{
   ...
   Promise.all([fetch('/releases/b0e0db9d9b0b547e08817056/assets/forge/'+name+'.mesh').then(r=>{if(!r.ok)throw Error('Prop unavailable: '+name);return r.arrayBuffer();}),loadTexture(g,name)]).then(([buffer,texture])=>this.staticMeshes.set(name,new Woodland(g,buffer,texture,sway))).catch(e=>console.warn(e));
  };
