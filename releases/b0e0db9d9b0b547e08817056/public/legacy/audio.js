...
export class Soundscape {
 ...
 async unlock(){if(!this.enabled)return;try{
  ...
  this.loading=true;const [manifest,response]=await Promise.all([fetch('/releases/b0e0db9d9b0b547e08817056/assets/audio/cues.json').then(r=>r.json()),fetch('/releases/b0e0db9d9b0b547e08817056/assets/audio/game-audio.mp3')]);this.cues=manifest;this.buffer=await this.ctx.decodeAudioData(await response.arrayBuffer());this.ready=true;
