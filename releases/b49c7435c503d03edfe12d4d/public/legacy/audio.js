...
export class Soundscape {
 ...
 async unlock(){if(!this.enabled)return;try{
  ...
  this.loading=true;const [manifest,response]=await Promise.all([fetch('/releases/b49c7435c503d03edfe12d4d/assets/audio/cues.json').then(r=>r.json()),fetch('/releases/b49c7435c503d03edfe12d4d/assets/audio/game-audio.mp3')]);this.cues=manifest;this.buffer=await this.ctx.decodeAudioData(await response.arrayBuffer());this.ready=true;
