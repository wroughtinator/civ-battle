...
import {branches,unitIcons,unitNames,counters,prerequisites,researchCost,researchGate,distances,route,abilities,canEnter} from './planning.js';
...
async function createRoom(){
 ...
 creating=(async()=>{const data=await api('/releases/b49c7435c503d03edfe12d4d/api/rooms',{seed,count,difficulty,civ,name:username,tag:callsign});room=data.room;token=data.token;slot=data.slot;seq=data.seq;saveSeat();history.replaceState({},'',`/?room=${room}`);await connect();})();
 try{await creating;}finally{creating=null;renderControls();}
}
async function joinRoom(id){room=id;const saved=stored(id);const data=await api(`/releases/b49c7435c503d03edfe12d4d/api/rooms/${id}/join`,{token:saved?.token,civ,name:username,tag:callsign});token=data.token;slot=data.slot;seq=data.seq;saveSeat();await connect();}
function connect(){
 clearTimeout(reconnectTimer);stopped=false;
 const ready=new Promise(resolve=>readyResolve=resolve);
 ws=new WebSocket(`${location.protocol==='https:'?'wss':'ws'}://${location.host}/releases/b49c7435c503d03edfe12d4d/api/rooms/${room}/ws`,['meridian',token]);
 ws.onmessage=e=>{
  ...
  if(msg.type==='state'){
   if(!msg.rules?.tactics){location.replace(`/releases/b49c7435c503d03edfe12d4d/legacy/index.html?room=${room}`);return;}
...
function aim(u,kind,value,iconName){
 ...
 if(!same){abilityTarget={icon:iconName,kind,value};document.body.classList.add('targeting');
  if(kind==='move'){const d=distances(world,u.tile);globe.setBlockedTargets(new Set(world.flatMap((t,i)=>state.tiles[i]?.visible&&d[i]<=3&&!canEnter(world,u.kind,i)?[i]:[])));}
...
try{
 ...
 const module=await WebAssembly.compileStreaming(fetch('/releases/b49c7435c503d03edfe12d4d/engine.wasm'));const e=new WebAssembly.Instance(module,{}).exports,enc=new TextEncoder(),dec=new TextDecoder();
