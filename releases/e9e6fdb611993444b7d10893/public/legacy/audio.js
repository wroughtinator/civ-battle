...
export class Soundscape {
 ...
 async unlock(){if(!this.enabled)return;try{
  ...
  this.loading=true;const [manifest,response]=await Promise.all([fetch('/releases/e9e6fdb611993444b7d10893/assets/audio/cues.json').then(r=>r.json()),fetch('/releases/e9e6fdb611993444b7d10893/assets/audio/game-audio.mp3')]);this.cues=manifest;this.buffer=await this.ctx.decodeAudioData(await response.arrayBuffer());this.ready=true;
