...
async function createRoom(){
 ...
 creating=(async()=>{const data=await api('/releases/e9e6fdb611993444b7d10893/api/rooms',{seed,count,difficulty,civ,name:username,tag:callsign});room=data.room;token=data.token;slot=data.slot;seq=data.seq;saveSeat();history.replaceState({},'',`/?room=${room}`);await connect();})();
 try{await creating;}finally{creating=null;renderControls();}
}
async function joinRoom(id){room=id;const saved=stored(id);const data=await api(`/releases/e9e6fdb611993444b7d10893/api/rooms/${id}/join`,{token:saved?.token,civ,name:username,tag:callsign});token=data.token;slot=data.slot;seq=data.seq;saveSeat();await connect();}
function connect(){
 clearTimeout(reconnectTimer);stopped=false;
 const ready=new Promise(resolve=>readyResolve=resolve);
 ws=new WebSocket(`${location.protocol==='https:'?'wss':'ws'}://${location.host}/releases/e9e6fdb611993444b7d10893/api/rooms/${room}/ws`,['meridian',token]);
 ws.onmessage=e=>{
  ...
  if(msg.type==='state'){
   if(!msg.rules?.tactics){location.replace(`/releases/e9e6fdb611993444b7d10893/legacy/index.html?room=${room}`);return;}
...
function renderResearch(){
 ...
 $('research').innerHTML=`<div class="panel-head">${icon('flask')}${btn('close-tech','close','Close research tree')}</div><div class="tree-scroll"><div class="connected-tree"><svg class="tree-links" viewBox="0 0 300 500" preserveAspectRatio="none" aria-hidden="true"><path d="M50 35V250M150 35V465M250 35V250M50 250V430L150 465M250 250V430L150 465"/></svg>${[0,1,2].map(row=>`<div class="tree-row">${branches.map(b=>node(b[row])).join('')}</div>`).join('')}<div class="tree-row final-nodes">${node(11)}</div><div class="tree-row final-nodes">${node(10)}</div></div></div>`;
...
async function leaveRoom(newLobby=false){
 ...
 try{
  ...
  const next=newLobby?await api('/releases/e9e6fdb611993444b7d10893/api/rooms',{seed:crypto.getRandomValues(new Uint32Array(1))[0],count,difficulty,civ,name:username,tag:callsign}):null;
...
try{
 ...
 const module=await WebAssembly.compileStreaming(fetch('/releases/e9e6fdb611993444b7d10893/engine.wasm'));const e=new WebAssembly.Instance(module,{}).exports,enc=new TextEncoder(),dec=new TextDecoder();
