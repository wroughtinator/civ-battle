...
export class Soundscape {
 ...
 async unlock(){if(!this.enabled)return;try{
  ...
  this.loading=true;const [manifest,response]=await Promise.all([fetch('/releases/65af3f525b76af60f10021f7/assets/audio/cues.json').then(r=>r.json()),fetch('/releases/65af3f525b76af60f10021f7/assets/audio/game-audio.mp3')]);this.cues=manifest;this.buffer=await this.ctx.decodeAudioData(await response.arrayBuffer());this.ready=true;
