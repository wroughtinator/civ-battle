...
export class Globe {
 ...
 setState(state,slot){const priorRadius=this.state?.cities?.find(c=>c.tile===this.selectedCity)?.radius;this.effects.accept(state);this.state=state;this.slot=slot;this.receivedAt=performance.now();this.preview=state.phase!=='running'&&state.phase!=='ended';this.targetOffset=this.preview&&innerWidth>760?.24:0;this.updateFog();this.rebuild();if(priorRadius!==state.cities?.find(c=>c.tile===this.selectedCity)?.radius)this.frameCity(this.selectedCity);}
 ...
 unitPosition(s,elapsed){if(s.locked_until>this.state.tick)elapsed=0;let p=this.world[s.tile].p,forward=null;if(s.path.length>1&&s.left>0){const f=Math.max(0,Math.min(1,1-Math.max(0,s.left-elapsed)/s.total)),a=this.world[s.path[0]].p,b=this.world[s.path[1]].p;p=norm(mix(a,b,f));forward=add(b,mul(a,-1));}return {p,forward};}
 updateUnits(now){const data=[],elapsed=Math.min(2,(now-(this.receivedAt||now))/1000);const tri=(a,b,c,col)=>{const n=norm(cross(add(b,mul(a,-1)),add(c,mul(a,-1))));for(const p of[a,b,c])data.push(...p,...n,...col);};
  ...
  for(const s of this.state?.squads||[]){const{p}=this.unitPosition(s,elapsed);const col=palette[s.owner]||[.75,.35,.22];ring(p,s.kind===9?.045:.024,col,this.world[s.tile].terrain===0?1.008:ground(p,this.world[s.tile].terrain===4?1:0)+.003);
