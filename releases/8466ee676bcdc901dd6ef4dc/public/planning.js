...
export function canEnter(world,k,i){return !!world[i]&&(k>=7&&k<=9?world[i].terrain===0:k===1||k===3?world[i].terrain!==4:true);}
...
export function route(world,state,u,to){
 if(!canEnter(world,u.kind,to))return null;const start=u.left&&u.path.length>1?u.path[1]:u.tile;
 ...
 if(!Number.isFinite(d[to]))return null;const path=[to];while(path[0]!==start)path.unshift(prev[path[0]]);if(start!==u.tile)path.unshift(u.tile);return {path,seconds:d[to]+(u.left||0)};
}
