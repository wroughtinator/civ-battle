...
async function createRoom(){
 ...
 creating=(async()=>{const data=await api('/releases/8466ee676bcdc901dd6ef4dc/api/rooms',{seed,count,difficulty,civ,name:username,tag:callsign});room=data.room;token=data.token;slot=data.slot;seq=data.seq;saveSeat();history.replaceState({},'',`/?room=${room}`);await connect();})();
 try{await creating;}finally{creating=null;renderControls();}
}
async function joinRoom(id){room=id;const saved=stored(id);const data=await api(`/releases/8466ee676bcdc901dd6ef4dc/api/rooms/${id}/join`,{token:saved?.token,civ,name:username,tag:callsign});token=data.token;slot=data.slot;seq=data.seq;saveSeat();await connect();}
function connect(){
 clearTimeout(reconnectTimer);stopped=false;
 const ready=new Promise(resolve=>readyResolve=resolve);
 ws=new WebSocket(`${location.protocol==='https:'?'wss':'ws'}://${location.host}/releases/8466ee676bcdc901dd6ef4dc/api/rooms/${room}/ws`,['meridian',token]);
 ws.onmessage=e=>{
  ...
  if(msg.type==='state'){
   if(!msg.rules?.tactics){location.replace(`/releases/8466ee676bcdc901dd6ef4dc/legacy/index.html?room=${room}`);return;}
...
function select(tile){
 ...
 const on=state.squads.find(u=>u.tile===tile&&!(u.left>0&&u.path.length>1));
...
function renderProvince(){
 ...
 if(u){
  ...
  $('province').innerHTML=`<div class="piece-head"><span class="piece-portrait" style="color:${colors[u.owner]}">${icon(unitIcons[u.kind])}</span><div><span class="owner-name">${html(state.players[u.owner]?.name||'')}</span><div class="health"><i style="width:${100*u.hp/sp.hp}%"></i></div>${stat('heart',num(u.hp))}</div><span class="spacer"></span>${city?btn('select-city','city','Select city here'):''}${btn('close-piece','close','Close selection')}</div>
   ...
   ${u.path.length>1?`<div class="movement-status">${stat('route',u.path.length-1)}${stat('clock',u.left)}</div>`:''}
...
function updateMarkers(now){if(!globe)return;for(const n of markerNodes){let pos=n.p;const u=n.u;if(u?.path.length>1&&u.left>0){const f=1-Math.max(0,u.left-(u.locked_until>state.tick?0:Math.min(2,(now-lastStateAt)/1000)))/u.total,a=world[u.path[0]].p,b=world[u.path[1]].p;pos=a.map((v,k)=>v*(1-f)+b[k]*f);const len=Math.hypot(...pos);pos=pos.map(v=>v/len);}const p=globe.project(pos);n.el.style.display=p.visible?'flex':'none';n.el.style.left=`${p.x}px`;n.el.style.top=`${p.y+(u?22:n.discovery?-15:-34)}px`;n.el.classList.toggle('selected',u?u.id===activeUnit:n.i===selected);n.el.classList.toggle('hurt',!!u&&feedbackLayer.timeline.items.some(e=>e.action==='hit'&&e.unit===u.id&&now-e.start<1200));}
...
try{
 ...
 const module=await WebAssembly.compileStreaming(fetch('/releases/8466ee676bcdc901dd6ef4dc/engine.wasm'));const e=new WebAssembly.Instance(module,{}).exports,enc=new TextEncoder(),dec=new TextDecoder();
