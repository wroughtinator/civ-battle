...
async function createRoom(){
 ...
 creating=(async()=>{const data=await api('/releases/3330eab767d4f5484d52392d/api/rooms',{seed,count,difficulty,civ,name:username,tag:callsign});room=data.room;token=data.token;slot=data.slot;seq=data.seq;saveSeat();history.replaceState({},'',`/?room=${room}`);await connect();})();
 try{await creating;}finally{creating=null;renderControls();}
}
async function joinRoom(id){room=id;const saved=stored(id);const data=await api(`/releases/3330eab767d4f5484d52392d/api/rooms/${id}/join`,{token:saved?.token,civ,name:username,tag:callsign});token=data.token;slot=data.slot;seq=data.seq;saveSeat();await connect();}
function connect(){
 clearTimeout(reconnectTimer);stopped=false;
 const ready=new Promise(resolve=>readyResolve=resolve);
 ws=new WebSocket(`${location.protocol==='https:'?'wss':'ws'}://${location.host}/releases/3330eab767d4f5484d52392d/api/rooms/${room}/ws`,['meridian',token]);
 ws.onmessage=e=>{
  ...
  if(msg.type==='state'){
   if(!msg.rules?.tactics){location.replace(`/releases/3330eab767d4f5484d52392d/legacy/index.html?room=${room}`);return;}
...
async function leaveRoom(newLobby=false){
 ...
 try{
  ...
  const next=newLobby?await api('/releases/3330eab767d4f5484d52392d/api/rooms',{seed:crypto.getRandomValues(new Uint32Array(1))[0],count,difficulty,civ,name:username,tag:callsign}):null;
...
try{
 ...
 const module=await WebAssembly.compileStreaming(fetch('/releases/3330eab767d4f5484d52392d/engine.wasm'));const e=new WebAssembly.Instance(module,{}).exports,enc=new TextEncoder(),dec=new TextDecoder();
