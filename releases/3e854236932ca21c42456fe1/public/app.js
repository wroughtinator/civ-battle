...
async function createRoom(){
 ...
 creating=(async()=>{const data=await api('/releases/3e854236932ca21c42456fe1/api/rooms',{seed,count,difficulty,civ,name:username,tag:callsign});room=data.room;token=data.token;slot=data.slot;seq=data.seq;saveSeat();history.replaceState({},'',`/?room=${room}`);await connect();})();
 try{await creating;}finally{creating=null;renderControls();}
}
async function joinRoom(id){room=id;const saved=stored(id);const data=await api(`/releases/3e854236932ca21c42456fe1/api/rooms/${id}/join`,{token:saved?.token,civ,name:username,tag:callsign});token=data.token;slot=data.slot;seq=data.seq;saveSeat();await connect();}
function connect(){
 clearTimeout(reconnectTimer);stopped=false;
 const ready=new Promise(resolve=>readyResolve=resolve);
 ws=new WebSocket(`${location.protocol==='https:'?'wss':'ws'}://${location.host}/releases/3e854236932ca21c42456fe1/api/rooms/${room}/ws`,['meridian',token]);
 ws.onmessage=e=>{
  ...
  if(msg.type==='state'){
   if(!msg.rules?.tactics){location.replace(`/releases/3e854236932ca21c42456fe1/legacy/index.html?room=${room}`);return;}
...
async function leaveRoom(newLobby=false){
 ...
 try{
  ...
  const next=newLobby?await api('/releases/3e854236932ca21c42456fe1/api/rooms',{seed:crypto.getRandomValues(new Uint32Array(1))[0],count,difficulty,civ,name:username,tag:callsign}):null;
...
function updateMarkers(now){if(!globe)return;for(const n of markerNodes){const u=n.u,pos=u?globe.unitPosition(u,now).p:n.p;const p=globe.project(pos);n.el.style.display=p.visible?'flex':'none';n.el.style.left=`${p.x}px`;n.el.style.top=`${p.y-10}px`;n.el.classList.toggle('selected',u?u.id===activeUnit:n.i===selected);n.el.classList.toggle('hurt',!!u&&feedbackLayer.timeline.items.some(e=>e.action==='hit'&&e.unit===u.id&&now-e.start<1200));}
...
try{
 ...
 const module=await WebAssembly.compile(await assetBytes('/releases/3e854236932ca21c42456fe1/engine.wasm'));const e=new WebAssembly.Instance(module,{}).exports,enc=new TextEncoder(),dec=new TextDecoder();
