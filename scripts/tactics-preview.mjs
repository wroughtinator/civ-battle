import {naval} from '../public/roster.js';
...
const state=run({op:'new',seed:31415,difficulty:1}).state;state.tick=1000;state.players.forEach(p=>p.bot=false);state.players[0].gold=3000;state.players[0].unlocked=Array.from({length:36},(_,i)=>i);
...
for(let k=0;k<36;k++){const tile=q.find(i=>d[i]>0&&d[i]<8&&(naval(k)?state.tiles[i].terrain===0:state.tiles[i].terrain===1)&&!state.squads.some(u=>u.tile===i));if(tile===undefined)continue;state.squads.push({...base,id:100+k,owner:0,kind:k,tile,to:tile,path:[tile],left:0,hp:run({op:'view',state,player:0}).state.rules.specs[k].hp,ready:0,ability_ready:0,refit:-1});state.tiles[tile].owner=0;}
...
if(process.env.TACTICS_PREVIEW_SCENE==='research'){state.players[0].unlocked=[0,12,13];state.players[0].gold=170;state.squads=state.squads.filter(u=>[0,12,13].includes(u.kind));state.tick=0;center.production=1;center.radius=1;state.discoveries=[];}
