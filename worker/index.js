  ...
  async webSocketMessage(ws, message) {
    ...
    await this.ctx.blockConcurrencyWhile(async()=>{
      ...
      if(msg.type==='leave') {
        ...
        else if(!(this.room.rulesVersion===4?['plan','face','move','stop','ability','attack','disembark','disband','explore','refit','research','train','upgrade']:['march','build','research','train','maneuver','strike','satellite','doctrine']).includes(msg.kind)||!['from','to','value'].every(k=>msg[k]===undefined||Number.isInteger(msg[k])&&msg[k]>=0&&msg[k]<(k==='value'?256:10000)))error=6;
